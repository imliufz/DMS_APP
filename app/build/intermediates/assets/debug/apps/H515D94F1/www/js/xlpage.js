...
function _tabCol(colorType,tabType,id,cusName,cusPhone,intentLevel,sex,isCheckbox,btnTxt,nextFunc1,nextFunc2,str1,str2,str3,str4,str5,str6,num,status,openId,wsId,buyCarIndex){
	...
	if (isCheckbox) {
		colBody +=  '		<td align="left" width="10%">'+
					'			<input type="checkbox" onclick="_checkBox(\''+tabType+'_'+id+'\')" value="'+tabType+'_'+id+'" id="'+tabType+'_'+id+'" name="userCheckBox" />'+numStr+'&nbsp;&nbsp;</a>'+
					'		</td>'+
	    ...
	    if(cusPhone!="" && cusPhone != null && cusPhone!="未留手机号"){
	    	...
	    	if(curFileName == "hallReceptionManage.html"){
	    		//展厅
	    		colBody += 	'		<a href="javascript:;"><img ico onClick="_setCallLog(3,'+id+',\''+cusPhone+'\');saveCallLog('+tabType+','+id+',\''+cusPhone+'\');window.location.href=\'tel:'+cusPhone+'\';" style="margin-left:-5px;width:25px;height:25px;" src="images/bohao.png"></a>';
	    	} else {
	    		//线索
	    		colBody += 	'		<a href="javascript:;"><img ico onClick="_setCallLog('+tabType+','+id+',\''+cusPhone+'\');saveCallLog('+tabType+','+id+',\''+cusPhone+'\');window.location.href=\'tel:'+cusPhone+'\';" style="margin-left:-5px;width:25px;height:25px;" src="images/bohao.png"></a>';
	    	}
	        colBody += '		<a href="javascript:;"><img ico phone="'+cusPhone+'" name="callMsg" style="margin-left:5px;width:25px;height:25px;" src="images/duanxing.png"></a>';
	        if(!IsNull(openId)){
	...
	'	<div class="weimob-list-item-image" style="background-image:url('+image+')"  onclick="'+nextFunc1+'"><input type="hidden" name="_page_list_hidden" id="_plh'+tabType+'_'+id+'" />'+
	'	</div>'+
